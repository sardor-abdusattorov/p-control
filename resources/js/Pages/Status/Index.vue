...
import DeleteBulk from "@/Pages/Status/DeleteBulk.vue";
...
watch(
    ...
    debounce(() => {
        ...
        router.get(route("status.index"), params, {
            replace: true,
            preserveState: true,
            preserveScroll: true,
        });
...
const selectAll = (event) => {
    if (event.target.checked === false) {
        ...
        props.statuses.data.forEach((status) => {
            data.selectedId.push(status.id);
        });
    }
};

const select = () => {
    data.multipleSelect = props.statuses.data.length === data.selectedId.length;
};
        ...
        <div class="space-y-4">
            <div class="px-4 sm:px-0">
                <div class="rounded-lg overflow-hidden w-fit">
                    <div>
                        <CreateLink :href="route('status.create')"/>
                    </div>
                    <Delete
            ...
            <div class="relative bg-white dark:bg-slate-800 shadow sm:rounded-lg">
                ...
                <div class="overflow-x-auto scrollbar-table">
                    <table class="w-full">
                        ...
                        <tbody>
                        <tr
                            v-for="(status, index) in props.statuses.data"
                            :key="index"
                        ...
                        >
                            ...
                            <td class="whitespace-nowrap py-4 px-2">
                                <div class="gap-1 justify-center overflow-hidden flex items-center">
                                    <ViewLink
                                        :href="route('status.show', { status: status.id })"
                ...
                <div class="flex justify-between items-center p-2 border-t border-slate-200 dark:border-slate-700">
                    <Pagination :links="props.statuses" :filters="data.params"/>
                </div>
            </div>
        </div>
    </AuthenticatedLayout>
