...
<template>
    <Dropdown align="right" width="100">
        <template #trigger>
            ...
            >
                <BellIcon
                    class="h-6 w-6 text-topbar-item hover:text-topbar-item-hover dark:text-zinc-300 dark:hover:text-white"/>
        ...
        <template #content>
            <div
                @click.stop
                class="absolute right-0 z-50 w-[20rem] lg:w-[26rem] bg-white dark:bg-zinc-800 rounded-md shadow-md mt-2 overflow-hidden"
            >
                <div class="p-4">
                    <h6 class="text-lg font-semibold mb-4 dark:text-white">{{ lang().label.notifications }}</h6>
                    <div class="max-h-[350px] overflow-y-auto">
                        <ul v-if="usePage().props.notifications.length > 0" class="divide-y divide-slate-200 dark:divide-zinc-600">
                            <li
                                ...
                                :key="notification.id"
                                class="flex items-start p-4 gap-3 hover:bg-slate-50 dark:hover:bg-zinc-700"
                            >
                                <div v-if="notification.type == 1">
                                    <Link @click.prevent="handleNotificationClick(notification.id)" :href="route('tasks.show', notification.task_id)">
                                        <div class="flex-grow">
                                            <h6 class="font-medium dark:text-white mb-4">{{ lang().notification.task_assigned }}</h6>
                                ...
                                <div v-if="notification.type == 2">
                                    <Link @click.prevent="handleNotificationClick(notification.id)" :href="route('tasks.show', notification.task_id)">
                                        <div class="flex-grow">
                                            <h6 class="font-medium dark:text-white mb-4">{{ lang().notification.task_completed }}</h6>
                                            <p class="text-xs text-slate-400 dark:text-zinc-400">
                                                {{ notification.created_at }}
                                            </p>
                                        </div>
                        ...
                        <p v-else class="text-center text-slate-500 dark:text-zinc-400 p-4">
