    ...
    <div class="text-slate-300 pb-20">
        ...
        <ul class="space-y-2 my-4">
            ...
            <li v-show="can(['manage products'])"
                ...
                class="text-white rounded-lg cursor-pointer flex items-center justify-between px-4 py-2 hover:bg-primary dark:hover:bg-primary"
                :class="expandedMenus.products.expanded ? 'bg-primary' : 'bg-slate-700/40 dark:bg-slate-800/40'">
                <div class="flex items-center">
                    <FolderIcon class="w-6 h-5" />
                    <span class="ml-3">{{ lang().label.products }}</span>
                </div>
                <ChevronDownIcon class="w-5 h-5 transform transition-transform" :class="{ 'rotate-180': expandedMenus.products.expanded }" />
            </li>
            <ul v-show="expandedMenus.products.expanded" class="space-y-1">
                <li :class="route().current('product_brands.*') ? 'bg-primary' : 'bg-slate-700/40 dark:bg-slate-800/40'"
                    class="text-white rounded-lg hover:bg-primary transition dark:hover:bg-primary">
                    <Link :href="route('product_brands.index')" class="flex items-center py-2 px-4">
                        <BriefcaseIcon class="w-6 h-5" />
                        <span class="ml-3">{{ lang().label.product_brands }}</span>
                    </Link>
                </li>
                <li :class="route().current('product_categories.*') ? 'bg-primary' : 'bg-slate-700/40 dark:bg-slate-800/40'"
                    class="text-white rounded-lg hover:bg-primary transition dark:hover:bg-primary">
                    <Link :href="route('product_categories.index')" class="flex items-center py-2 px-4">
                        <FolderIcon class="w-6 h-5" />
                        <span class="ml-3">{{ lang().label.product_categories }}</span>
                    </Link>
                </li>
                <li :class="route().current('products.*') ? 'bg-primary' : 'bg-slate-700/40 dark:bg-slate-800/40'"
                    class="text-white rounded-lg hover:bg-primary transition dark:hover:bg-primary">
                    <Link :href="route('products.index')" class="flex items-center py-2 px-4">
                        <ClipboardDocumentListIcon class="w-6 h-5" />
                        <span class="ml-3">{{ lang().label.products }}</span>
                    </Link>
                </li>
